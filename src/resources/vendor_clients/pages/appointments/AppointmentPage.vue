        ...
        <div>
            <v-data-table-server :headers="headers" :items="appointments" :items-length="total" :search="search"
                ...
                <template #top>
                    <v-toolbar flat class="px-4 bg-white py-4">
                        <v-container fluid class="d-flex pa-0 align-center">
                            ...
                            <v-select v-model="selectedStatus" variant="outlined" density="compact" :items="statusOptions"
                                label="Status" dense outlined class="mx-2" hide-details style="max-width: 200px"></v-select>
                ...
                <template v-slot:[`item.actions`]="{ item }">
                    <v-btn icon size="x-small" variant="tonal" color="primary" :to="{ name: 'globalAppointmentDetailPage', params: { id: item.id } }" elevation="0"
