<template>
    <v-card>
        <v-card-title v-if="embedded" class="d-flex justify-space-between align-center py-0">
            <div class="d-flex align-center">
                <v-btn v-if="showBack" icon variant="text" color="primary" @click="$emit('onBack')">
                    <v-icon>mdi-arrow-left</v-icon>
                </v-btn>
                <h4 class="mb-0">{{ title }}</h4>
            </div>
            <v-btn icon variant="text" color="error" @click="$emit('onClose')">
                <v-icon>mdi-close</v-icon>
            </v-btn>
        </v-card-title>
        <v-card-title v-else class="d-flex justify-space-between align-center py-0">
            <h4 class="mb-0">{{ title }}</h4>
            <v-btn icon variant="text" color="error" @click="$emit('onClose')">
                <v-icon>mdi-close</v-icon>
            </v-btn>
        ...
        <v-card-text class="mt-4">
            <v-form ref="formRef" v-model="valid" lazy-validation>
                <v-row dense>
                    <v-col cols="12">
                        <v-text-field v-model="customer.customer_name" label="Name on Card" :rules="nameRules" required
            ...
            <div class="pt-2">
                ...
                <div class="pt-4 text-center">
                    <v-btn rounded color="success" size="x-large" :disabled="loading" @click="handleSubmit">
...
const props = defineProps({
    title: { type: String, default: "Add New Card" },
...
async function handleSubmit() {
    ...
    console.log({ paymentMethod })
        ...
        const response = await $axios.post(`/customers/add-payment-method`, {
